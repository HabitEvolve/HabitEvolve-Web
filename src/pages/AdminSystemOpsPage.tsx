...
const pickerInputCls =
    "w-full border-2 border-[#1A1D20] rounded-lg pl-9 pr-3 py-2 text-sm font-medium " +
    "focus:outline-none focus:ring-2 focus:ring-[#f7a561] dark:bg-gray-800 dark:text-gray-100";
...
function UserPicker({
    ...
    if (value) {
        return (
            <div className="flex items-center justify-between gap-2 border-2 border-[#1A1D20] rounded-lg px-3 py-2 bg-[#eef4ff] dark:bg-gray-800">
                <div className="min-w-0">
                    <p className="text-sm font-black text-[#1A1D20] dark:text-gray-100 truncate">{value.username}</p>
                    <p className="text-xs text-[#1A1D20]/60 dark:text-gray-400 truncate">{value.email}</p>
                </div>
                <button
                    type="button"
                    onClick={() => onChange(null)}
                    className="shrink-0 w-6 h-6 flex items-center justify-center rounded-full hover:bg-black/10 dark:hover:bg-white/10"
                >
    ...
    return (
        <div className="relative">
            <div className="relative">
                <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-[#1A1D20]/40" />
                <input
                    value={query}
            ...
            {open && query.trim() && (
                <div className="absolute z-20 mt-1 w-full bg-white dark:bg-gray-800 border-2 border-[#1A1D20] rounded-lg shadow-[3px_3px_0_0_#555] max-h-48 overflow-y-auto">
                    {searching ? (
                        <div className="px-3 py-2 text-xs text-[#1A1D20]/50 dark:text-gray-400 flex items-center gap-2">
                            <Loader2 className="w-3.5 h-3.5 animate-spin" /> Searching...
                        </div>
                    ) : results.length === 0 ? (
                        <div className="px-3 py-2 text-xs text-[#1A1D20]/50 dark:text-gray-400">No users found.</div>
                    ) : results.map(u => (
                        <button
                            type="button"
                            key={u.userId}
                            onMouseDown={() => { onChange(u); setQuery(""); setOpen(false); }}
                            className="w-full text-left px-3 py-2 text-sm hover:bg-[#fde8c8] dark:hover:bg-gray-700 border-b border-[#1A1D20]/10 last:border-0"
                        >
                            <div className="font-bold text-[#1A1D20] dark:text-gray-100">{u.username}</div>
                            <div className="text-xs text-[#1A1D20]/50 dark:text-gray-400">{u.email}</div>
                        </button>
                    ))}
                </div>
            )}
            ...
            {value.length > 0 && (
                <div className="flex flex-wrap gap-1.5">
                    {value.map(u => (
                        <span
                            key={u.userId}
                            className="inline-flex items-center gap-1.5 pl-2.5 pr-1.5 py-1 rounded-lg border-2 border-[#1A1D20] bg-[#eef4ff] dark:bg-gray-800 text-xs font-black text-[#1A1D20] dark:text-gray-100"
                        >
                            {u.username}
                            <button
                                type="button"
                                onClick={() => removeUser(u.userId)}
                                className="w-4 h-4 flex items-center justify-center rounded-full hover:bg-black/15 dark:hover:bg-white/15"
                            >
                                <X className="w-3 h-3" />
                            </button>
                        </span>
                    ...
                    <div className="absolute z-20 mt-1 w-full bg-white dark:bg-gray-800 border-2 border-[#1A1D20] rounded-lg shadow-[3px_3px_0_0_#555] max-h-48 overflow-y-auto">
                        {searching ? (
                            ...
                            <button
                                ...
                                className="w-full text-left px-3 py-2 text-sm hover:bg-[#fde8c8] dark:hover:bg-gray-700 border-b border-[#1A1D20]/10 last:border-0 disabled:opacity-40"
                            >
                                <div className="font-bold text-[#1A1D20] dark:text-gray-100">{u.username}</div>
                                <div className="text-xs text-[#1A1D20]/50 dark:text-gray-400">{u.email}</div>
                            </button>
...
function PartyPicker({
    ...
    if (value) {
        return (
            <div className="flex items-center justify-between gap-2 border-2 border-[#1A1D20] rounded-lg px-3 py-2 bg-[#eef4ff] dark:bg-gray-800">
                <div className="min-w-0">
                    <p className="text-sm font-black text-[#1A1D20] dark:text-gray-100 truncate">{value.name}</p>
                    {value.status && <p className="text-xs text-[#1A1D20]/60 dark:text-gray-400 truncate">{value.status}</p>}
                </div>
                <button
                    type="button"
                    onClick={() => onChange(null)}
            ...
            {open && query.trim() && (
                <div className="absolute z-20 mt-1 w-full bg-white dark:bg-gray-800 border-2 border-[#1A1D20] rounded-lg shadow-[3px_3px_0_0_#555] max-h-48 overflow-y-auto">
                    {searching ? (
                        ...
                        <div className="px-3 py-2 text-xs text-[#1A1D20]/50 dark:text-gray-400">No parties found.</div>
                    ) : results.map(p => (
                        <button
                            type="button"
                            key={p.partyId}
                            ...
                            className="w-full text-left px-3 py-2 text-sm hover:bg-[#fde8c8] dark:hover:bg-gray-700 border-b border-[#1A1D20]/10 last:border-0"
                        >
                            <div className="font-bold text-[#1A1D20] dark:text-gray-100">{p.name}</div>
                            {p.status && <div className="text-xs text-[#1A1D20]/50 dark:text-gray-400">{p.status}</div>}
                        </button>
                    ))}
                </div>
            )}
        ...
        <div className="border-4 border-[#1A1D20] rounded-2xl shadow-[4px_4px_0_0_#1A1D20] overflow-hidden">
            <div className="bg-[#e0d4f7] px-5 py-4 flex items-center gap-3 border-b-4 border-[#1A1D20]">
                ...
                <div>
                    ...
                    <p className="text-xs font-medium text-[#1A1D20]/70">Send to ALL users, a ROLE group, or specific users</p>
                </div>
            </div>
            <div className="bg-white dark:bg-gray-900 px-5 py-5 space-y-4">
                <div>
                ...
                {target === "USERS" && (
                    <div>
                        <label className="block text-xs font-black text-[#1A1D20] dark:text-gray-300 mb-1">Users *</label>
...
function InAppPanel() {
    ...
    return (
        <div className="border-4 border-[#1A1D20] rounded-2xl shadow-[4px_4px_0_0_#1A1D20] overflow-hidden">
            ...
            <div className="bg-white dark:bg-gray-900 px-5 py-5 space-y-4">
                <div>
                    <label className="block text-xs font-black text-[#1A1D20] dark:text-gray-300 mb-1">User *</label>
                ...
                {result && (
                    <div className="bg-[#C8F7DC] border-2 border-[#1A1D20] rounded-xl p-4 space-y-1">
                        ...
                        <div className="text-xs font-medium text-[#1A1D20]/80 space-x-2">
                            ...
                            <span>·</span>
                            <span><span className="font-black">User:</span> {user?.username}</span>
                            <span>·</span>
...
function ProofOverridePanel() {
    ...
    return (
        <div className="border-4 border-[#1A1D20] rounded-2xl shadow-[4px_4px_0_0_#1A1D20] overflow-hidden">
            ...
            <div className="bg-white dark:bg-gray-900 px-5 py-5 space-y-4">
                <div>
                    <label className="block text-xs font-black text-[#1A1D20] dark:text-gray-300 mb-1">Proof Submitter *</label>
                    <UserPicker value={targetUser} onChange={setTargetUser} placeholder="Search the user who submitted the proof..." />
                </div>

                {targetUser && (
                    <div>
                        <label className="block text-xs font-black text-[#1A1D20] dark:text-gray-300 mb-1">Proof *</label>
                        {proofsLoading ? (
                            <div className="flex items-center gap-2 text-xs text-[#1A1D20]/50 dark:text-gray-400 py-2">
                                <Loader2 className="w-3.5 h-3.5 animate-spin" /> Loading proofs...
                            </div>
                        ) : !proofs || proofs.length === 0 ? (
                            <div className="text-xs text-[#1A1D20]/50 dark:text-gray-400 py-2">No proofs found for this user.</div>
                        ) : (
                            <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1 border-2 border-[#1A1D20]/15 rounded-lg p-2">
                                {proofs.map(p => (
                                    <button
                                        type="button"
                                        key={p.proofId}
                                        onClick={() => setSelectedProofId(p.proofId)}
                                        className={`w-full text-left px-3 py-2 rounded-lg border-2 transition-all ${selectedProofId === p.proofId
                                            ? "border-[#1A1D20] bg-[#fde8c8]"
                                            : "border-transparent hover:bg-gray-100 dark:hover:bg-gray-800"
                                            }`}
                                    >
                                        <div className="flex items-center justify-between gap-2">
                                            <span className="text-sm font-bold text-[#1A1D20] dark:text-gray-100 truncate">
                                                {p.questTitle ?? p.proofType}
                                            </span>
                                            <span className="text-[10px] font-black uppercase text-[#1A1D20]/50 dark:text-gray-400 shrink-0">
                                                {p.status}
                                            </span>
                                        </div>
                                        <div className="text-[10px] text-[#1A1D20]/50 dark:text-gray-400">
                ...
                <p className="text-[10px] font-semibold text-[#1A1D20]/50 dark:text-gray-400">
                    Acting as: <span className="font-black text-[#1A1D20] dark:text-gray-200">{currentAdmin?.username ?? "—"}</span> (you)
                </p>
                {err && (
                    <div className="flex items-center gap-2 bg-[#FFD6D6] border-2 border-[#1A1D20] rounded-lg px-4 py-2">
                        <AlertCircle className="w-4 h-4 text-red-700 shrink-0" />
        ...
        <div className="border-4 border-[#1A1D20] rounded-2xl shadow-[4px_4px_0_0_#1A1D20] overflow-hidden">
            <div className="bg-[#d4eeff] px-5 py-4 flex items-center gap-3 border-b-4 border-[#1A1D20]">
                ...
                <div>
                    ...
                    <p className="text-xs font-medium text-[#1A1D20]/70">Manually apply penalty or restore HP for a party's raid</p>
                </div>
            </div>
            <div className="bg-white dark:bg-gray-900 px-5 py-5 space-y-4">
                <div>
                    <label className="block text-xs font-black text-[#1A1D20] dark:text-gray-300 mb-1">Party *</label>
                ...
                {party && (
                    statusLoading ? (
                        ...
                        <div className="flex items-center gap-2 bg-[#FFD6D6] border-2 border-[#1A1D20] rounded-lg px-4 py-2">
                            <AlertCircle className="w-4 h-4 text-red-700 shrink-0" />
                            <span className="text-xs font-black text-[#8b0000]">{statusError}</span>
                        </div>
                    ) : bossStatus ? (
                        <div className="bg-[#f3f7ff] dark:bg-gray-800 border-2 border-[#1A1D20]/20 rounded-xl p-3 space-y-2">
                            <div className="flex items-center gap-2">
                                <Swords className="w-4 h-4 text-[#1A1D20]/60" />
                                <span className="text-sm font-black text-[#1A1D20] dark:text-gray-100">{bossStatus.bossName}</span>
                                <span className="text-[10px] font-bold text-[#1A1D20]/50 dark:text-gray-400 uppercase">Raid #{bossStatus.raidId}</span>
                            </div>
                            <div className="flex justify-between text-xs font-medium text-[#1A1D20]/80 dark:text-gray-300">
                                <span>HP</span>
                                <span className="font-black">{bossStatus.currentHp} / {bossStatus.maxHp}</span>
                            </div>
                            <div className="w-full h-2 bg-[#1A1D20]/10 rounded-full overflow-hidden border border-[#1A1D20]/20">
                                <div className="h-full bg-[#1a6b3a] rounded-full" style={{ width: `${bossStatus.hpPercent}%` }} />
                            </div>
                        </div>
                    ) : null
                )}
                ...
                <div className="grid grid-cols-2 gap-3">
                    <button
                        ...
                        disabled={running !== null || !bossStatus}
