...
const LOADING_LABEL: Record<AuthMode, string> = {
    login: 'Connecting...',
    register: 'Connecting...',
};
    ...
    const handleClick = async () => {
        ...
        try {
            const { error: oauthError } = await supabase.auth.signInWithOAuth({
                provider: 'google',
                options: {
                    // Both login and register land on the same callback page.
                    // AuthContext handles the BE upsert and stores the JWT.
                    redirectTo: `${window.location.origin}/auth/callback`,
                    queryParams: {
                        // Force Google account picker so the user can switch accounts
            ...
            // keep isLoading=true so the button stays in loading state during redirect.
        } catch (err: any) {
